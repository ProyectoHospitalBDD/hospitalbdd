...
BEGIN
  ...
  -- =========================================================
  -- NUEVO: Doctor debe estar ACTIVO (empleado.estatus = 1)
  -- =========================================================
  IF NOT EXISTS (
    SELECT 1
    FROM dbo.doctor d
    JOIN dbo.empleado e ON e.idUsuario = d.idUsuario
    WHERE d.idUsuario = @DoctorId
      AND e.estatus = 1
  )
    THROW 51008, 'DoctorInactivo', 1;
  ...
  BEGIN TRY
    ...
    -- 1) El paciente no puede tener otra cita pendiente con el mismo doctor
    IF EXISTS(
      SELECT 1
    ...
    IF EXISTS(
      ...
      WHERE c.idDoctor = @DoctorId
        AND c.estatusCita IN (N'AgendadaPendPago', N'PagadaPendAtender',N'CancelacionSolicitadaDoctor')
        AND c.fechaHoraFin  > @FechaInicio
        AND c.fechaHoraInicio < @FechaFin
    )
      THROW 51003, 'DoctorOcupado', 1;

    -- 3) El paciente no puede traslaparse con nadie (aunque sea otro doctor)
    IF EXISTS(
      SELECT 1
      FROM dbo.cita c WITH (UPDLOCK, HOLDLOCK)
      WHERE c.idPaciente = @PacienteId

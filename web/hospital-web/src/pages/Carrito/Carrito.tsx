...
import { useCart, CartItem, getCartKey } from "./CartContext";
import {
  registrarCompra,
  CrearCompraPayload,
...
export default function Carrito() {
  ...
  const obtenerDatosDelToken = (token: string) => {
    try {
      ...
      if (nombre || email) setFormData((prev) => ({ ...prev, nombre, email }));
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const imprimirTicket = (
    ...
    if (ventana) {
      ...
      alert("Habilita ventanas emergentes para ver el ticket.");
    }
  };
      ...
      <div className="carrito-container">
        <div className="carrito-items-list">
          {cart.map((item) => {
            ...
            const limite = item.stock !== null && item.stock !== undefined ? item.stock : null;
            const puedeSubir = limite === null ? true : item.cantidad < limite;
            ...
            return (
              <div key={key} className="carrito-item">
                ...
                <div className="cart-controls">
                  <button onClick={() => updateQuantity(key, item.cantidad - 1)}>
                    -
                  </button>
                  ...
                  <button
                    ...
                    disabled={!puedeSubir}
                    title={!puedeSubir ? "Sin stock/cupo" : ""}
                    style={!puedeSubir ? { opacity: 0.5, cursor: "not-allowed" } : undefined}
                  >
                    +
                  </button>
                </div>
                ...
                <button className="cart-remove" onClick={() => removeFromCart(key)}>
                  ✕
                </button>
              </div>
            );
        ...
        <div className="checkout-modal-backdrop">
          <div className="checkout-modal">
            ...
            <p style={{ fontSize: "0.9rem", color: "#666", marginBottom: "20px" }}>
              Por favor confirma tus datos para la entrega.
            </p>

...
import { getMisCitasDoctor, type CitaDoctor, getHistorialMedicoPaciente, type HistorialMedicoPaciente } from "../../../api/doctorApi";
...
export default function DoctorAtenderCitaPage() {
  ...
  const citasDelPaciente = useMemo(() => {
    ...
    return citas.filter((c) => c.idPaciente === selectedPacienteId);
    ...
    <div className="page">
      <div className="carta">
        ...
        <div style={{ margin: "12px 0" }}>
          <label style={{ display: "block", marginBottom: 6 }} className="etiqueta">Pacientes con citas pendientes</label>
          {loading ? (
            <div>Cargando...</div>
          ) : (
            <select
            ...
            {citasDelPaciente.length === 0 ? (
              ...
              <ul className="etiqueta">
                {citasDelPaciente.map((c) => {
                  const est = (c.estatus ?? "").toLowerCase();
                  if (est !== "pagadapendatender") return null;
                  return (
                    <li key={c.idCita} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span>{c.fecha} · {c.horaInicio}{c.horaFin ? `–${c.horaFin}` : ""} · {c.estatus}</span>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button onClick={() => navigate('/receta', { state: { citaId: c.idCita, pacienteId: c.idPaciente } })} className="boton">
                          Generar Receta
                        </button>
        ...
        {mostrarHistorial && historialMedico && (
          ...
          className="espacio">
            ...
            <p className="etiqueta"><strong>Tipo de Sangre:</strong> {historialMedico.tipoSangre || "No registrado"}</p>
            <p className="etiqueta"><strong>Peso:</strong> {historialMedico.peso ? `${historialMedico.peso} kg` : "No registrado"}</p>
            <p className="etiqueta"><strong>Estatura:</strong> {historialMedico.estatura ? `${historialMedico.estatura} m` : "No registrado"}</p>
          </div>
        )}

        <p className="texto">Aquí podrás seleccionar la cita y comenzar el proceso de atención (receta, notas, etc.).</p>
      </div>
    </div>
  );
}

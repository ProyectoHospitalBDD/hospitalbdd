...
import "./FarmaceuticoPerfilPage.css";
import { useEffect, useState } from "react";
import { getMiPerfilFarmaceutico, type PerfilFarmaceutico } from "../../api/farmaceuticoApi";

export default function FarmaceuticoPerfilPage() {
  const [perfil, setPerfil] = useState<PerfilFarmaceutico | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      setError("");
      try {
        const p = await getMiPerfilFarmaceutico();
        setPerfil(p);
      } catch (e: any) {
        setError(e?.message ?? "Error cargando perfil del farmacéutico");
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <div className="perfil-page">
      <div className="perfil-card">
        <h2 className="perfil-title">Mi perfil (Farmacéutico)</h2>
